describe("Blog app", function () {
  ...
  describe("When logged in", function () {
    ...
    describe("When a blog has been created", function () {
      ...
      it("Only the authorised user can see a blog's delete button", function () {
    ...
    describe("When multiple blogs have been created", function () {
      ...
      it("Blogs are arranged by number of likes", function () {
        cy.get('.blog').eq(0).should('contain', 'Blog Title 2 by Another')
        cy.get('.blog').eq(1).should('contain', 'Blog Title 3 by Blog Author')
        cy.get('.blog').eq(2).should('contain', 'Blog Title by Blog Author')

        cy.get('.blog').eq(2).contains('view').click()
        cy.get('.blog').eq(2).contains('like').click()
        cy.get('.blog').eq(1).contains('Blog Title by Blog Author')
        cy.get('.blog').eq(1).contains('like').click()

        cy.get('.blog').eq(0).should('contain', 'Blog Title by Blog Author')
        cy.get('.blog').eq(1).should('contain', 'Blog Title 2 by Another')
        cy.get('.blog').eq(2).should('contain', 'Blog Title 3 by Blog Author')
      })
    })
  })
})